...
强制等待：强制等待，线程休眠一定时间，time.sleep()【不灵活，能不用就尽量不用】

隐式等待：设置一个等待时间，轮询（默认0.5秒）查找元素是否出现，如果没有出现就抛异常， self.driver.implicitly_wait()

显式等待：1.在代码中定义条件，当条件发生时才继续执行代码
        2.‘WebDriverWait’ 配合until()和until_not()方法，根据判断条件进行等待
        3.程序每隔一段时间（默认0.5秒）进行条件判断，如果条件成立，则执行下一步，否则继续等待，知道超出设置的最长时间
...
class TestWait:
    ...
    # def test_a(self):
    #     self.driver.find_element_by_link_text('所有分类').click()
    #     self.num=self.driver.find_element_by_class_name("table-heading")
